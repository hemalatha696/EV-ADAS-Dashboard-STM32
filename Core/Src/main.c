...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim->Instance == TIM1)
        flag_ev = 1;      /* 10ms EV tick */

    if(htim->Instance == TIM3)
        sensor_flag = 1;  /* 100ms ultrasonic tick */
}
...
int main(void)
{
  ...
  ADAS_Init( &adas); //initialize l r f -> 400cm
  Fault_Init(&flt);
  Shell_Init(&huart1, &ev, &adas, &flt );// flag-> none, active = 0
